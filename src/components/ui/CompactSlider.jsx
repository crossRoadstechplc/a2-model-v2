...
export function CompactSlider({
  ...
  return (
    <div className="px-3 py-3 rounded-lg border border-slate-200 bg-slate-50/60">
      {/* Label row */}
      <div className="flex items-center justify-between mb-1.5">
        <p className="text-[11px] font-semibold text-slate-700 leading-tight">{label}</p>
        <span className="text-[11px] font-bold text-slate-800 tabular-nums ml-3 shrink-0 bg-white border border-slate-200 px-2 py-0.5 rounded-md shadow-sm">
          {format(value)}
        </span>
      </div>
      ...
      <input
        ...
        className={clsx('w-full h-1.5 rounded-full appearance-none cursor-pointer', accent)}
      />
